...
- Added a shared ownership-scoped dense `RetrievalService` used by chat and the offline evaluator.
- Added strict JSONL dataset validation, deterministic Hit@1/3/5, MRR, Recall@5, and latency metrics.
- Added reusable grounded generation execution and optional RAGAS 0.4.3 collections metrics for answerable cases only, with separate refusal analysis for unanswerable cases.
- Added a bounded-concurrency CLI that resolves owned ready PDFs by filename and writes partial JSON plus CSV results. Evaluation is offline and is never invoked during startup or normal chat requests.
- Added labelling, cost, reliability, fair-comparison, manual-review, and baseline-preservation guidance under `backend/evaluation/`.

## Evaluation verification in this runner
- `ruff check app tests`, `ruff format --check app tests`, `mypy app`, `python -m compileall -q app tests`, and `git diff --check` pass from `backend/`.
- A Python 3.11 virtual environment was created, but `pip install -e .` was blocked because the package proxy returned `403 Forbidden` while resolving build dependencies. Consequently `pytest` cannot import FastAPI, Pydantic, SQLAlchemy, PyMuPDF, PyJWT, or RAGAS in this runner.
- PostgreSQL/pgvector integration and live Gemini/RAGAS judge calls remain manual. No evaluation score has been fabricated.
